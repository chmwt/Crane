...
motor::M3508 motor_z(2);
motor::M2006 motor_y(1);
...
void move_init(void)
{
  while (!motor_y.is_open() || !motor_z.is_open()) {
    vTaskDelay(1);
  }
  uint8_t init_time = 0;
  while (1) {
...
void move_control_loop(void)
{
  ...
  //   plotter.plot(set, pos_now.xl, -pos_now.xr);
  ...
  if (pos_set.y > 0.27) pos_set.y = 0.27;
  if (pos_set.y < -0.47) pos_set.y = -0.47;
